...
/* Dates follow the language on <html>, which lib/i18n.js writes before the
   first paint. `ar-u-nu-latn` gives Arabic month and weekday names with Latin
   digits — the digits have to keep lining up with the money column beside
   them, and every price in this app is Latin. Money itself never moves. */
const dtLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* "3m ago" is a sentence, so it comes from the app's dictionary rather than
   from here. These are the English defaults, replaced at boot. */
let relText = {
  now: () => 'just now',
  min: (n) => `${n}m ago`,
  hour: (n) => `${n}h ago`,
...
export function setRelativeText(next) { Object.assign(relText, next); }

let dialogText = { title: 'Confirm', ok: 'Confirm', cancel: 'Cancel', close: 'Close dialog' };
export function setDialogText(next) { Object.assign(dialogText, next); }

export function fmtDate(d, opts) {
  const dt = d instanceof Date ? d : new Date(d);
  return dt.toLocaleDateString(dtLocale(), opts || { day: '2-digit', month: 'short', year: 'numeric' });
}
export function fmtTime(d) {
  const dt = d instanceof Date ? d : new Date(d);
  return dt.toLocaleTimeString(dtLocale(), { hour: '2-digit', minute: '2-digit', hour12: false });
}
export function ago(d) {
  const s = (Date.now() - new Date(d).getTime()) / 1000;
  if (s < 60) return relText.now();
  if (s < 3600) return relText.min(Math.floor(s / 60));
  if (s < 86400) return relText.hour(Math.floor(s / 3600));
  if (s < 2592000) return relText.day(Math.floor(s / 86400));
  return fmtDate(d);
}
...
export function modal({ title, body, actions = [], width }) {
  ...
  box.appendChild(h('div', { class: 'modal__head' },
    ...
    h('button', {
      ...
      'aria-label': dialogText.close, title: dialogText.close,
...
export function confirmDialog(message, { title, danger = false, okLabel } = {}) {
  return new Promise((resolve) => {
    modal({
      title: title ?? dialogText.title,
      body: `<p class="muted">${esc(message)}</p>`,
      actions: [
        { label: dialogText.cancel, class: '', onClick: () => resolve(false) },
        { label: okLabel ?? dialogText.ok, class: danger ? 'btn--danger' : 'btn--primary', onClick: () => resolve(true) },
      ],
    });
  });
}
