...
export function initPWA({ mount, appName = 'this app', swPath = './sw.js', onNote, labels } = {}) {
  const T = Object.assign({}, PWA_TEXT, labels || {});
  ...
  btn.innerHTML = `${ICON}<span>${T.install}</span>`;
  btn.hidden = !isIOS();               // shown at once on iOS, on prompt elsewhere
  btn.title = T.title(appName);
  ...
  window.addEventListener('appinstalled', () => {
    deferred = null;
    btn.hidden = true;
    if (onNote) onNote(T.installed(appName));
  });
  ...
  btn.addEventListener('click', async () => {
    if (deferred) {
      ...
      if (outcome !== 'accepted' && onNote) onNote(T.dismissed);
      deferred = null;
      return;
    }
    if (onNote) {
      onNote(isIOS() ? T.ios : T.menu);
    }
  });
