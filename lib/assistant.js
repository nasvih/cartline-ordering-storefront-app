...
export class Assistant {
  constructor(cfg) {
    this.cfg = Object.assign({
      ...
      context: () => ({}),
      /* Every word this class puts on the screen itself. The app hands its
         own dictionary in, so the panel speaks the language the page does. */
      labels: {},
    }, cfg);
  ...
  mount(root = document.body) {
    ...
    fab.type = 'button';
    fab.title = this.T.fabTitle(this.cfg.name);
    fab.setAttribute('aria-label', this.T.openAria(this.cfg.name));
    ...
    host.innerHTML = `
      <section class="assist${docked ? ' assist--docked' : ''}" role="dialog" aria-label="${esc(this.cfg.name)}">
        <header class="assist__head">
          ...
          <button class="btn btn--ghost btn--icon" data-act="reset" title="${esc(this.T.reset)}" aria-label="${esc(this.T.reset)}">${ICON.reset}</button>
          ${docked ? '' : `<button class="btn btn--ghost btn--icon" data-act="close" title="${esc(this.T.close)}" aria-label="${esc(this.T.close)}">${ICON.close}</button>`}
        </header>
        <div class="assist__log" data-log></div>
        <div class="assist__suggest" data-suggest></div>
        <form class="assist__form" data-form>
          <input class="input" placeholder="${esc(this.T.ask)}" aria-label="${esc(this.T.ask)}" autocomplete="off" data-input>
          <button class="btn btn--primary btn--icon" type="submit" title="${esc(this.T.send)}" aria-label="${esc(this.T.send)}">${ICON.send}</button>
        </form>
  ...
  _push(who, html, meta) {
    ...
    el.innerHTML = `<span class="msg__av">${who === 'me' ? esc(this.T.you) : esc(this.cfg.initials)}</span>
  ...
  async ask(q) {
    ...
    catch (err) { out = { text: this.T.edge }; }
  ...
  _renderActions(body, actions) {
    ...
    actions.forEach((a, i) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        btn.textContent = a.doingLabel || this.T.working;
        let result;
        try { result = await a.run(); } catch (err) { result = { text: this.T.failed }; }
        if (typeof result === 'string') result = { text: result };
        row.remove();
        const done = this._push('bot', '', '');
        done.innerHTML = render(result.text || this.T.done);
        if (result.table) done.innerHTML += tableHTML(result.table);
        done.innerHTML += `<div class="msg__meta">${esc(result.meta || this.T.appliedMeta)}</div>`;
  ...
  _route(q) {
    ...
    if (best) return best;
    return { answer: () => ({ text: pick(this.cfg.fallbacks), suggestions: this.cfg.suggestions }), trace: this.T.searchTrace };
  }
}
